...
async def test_openapi_exposes_every_documented_path(client: httpx.AsyncClient) -> None:
...
async def test_search_takes_q_and_exposes_no_limit_parameter(client: httpx.AsyncClient) -> None:
    schema = (await client.get("/openapi.json")).json()
...
async def test_bearer_security_scheme_is_declared(client: httpx.AsyncClient) -> None:
    schema = (await client.get("/openapi.json")).json()
