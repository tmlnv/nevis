...
from tests.custom_types import Build, Fixture, JsonValue
...
@pytest.fixture
def build(settings: Settings) -> Build:
...
def _json(status: int, payload: JsonValue, headers: dict[str, str] | None = None) -> httpx.Response:
    return httpx.Response(status, content=orjson.dumps(payload), headers=headers)


async def test_happy_path_returns_vectors_and_model(build: Fixture[Build]) -> None:
...
async def test_summarize_strips_and_returns_content(build: Fixture[Build]) -> None:
...
async def test_status_codes_map_to_documented_errors(
    build: Fixture[Build], status: int, code: str, mapped: int, is_retryable: bool, attempts: int
) -> None:
...
async def test_rate_limit_carries_retry_after_and_is_not_retried(build: Fixture[Build]) -> None:
...
async def test_non_numeric_retry_after_is_ignored(build: Fixture[Build]) -> None:
    ai, _ = build(
...
async def test_absurd_retry_after_is_clamped(build: Fixture[Build]) -> None:
...
async def test_unusable_success_body_maps_to_invalid_response(
    build: Fixture[Build], payload: dict[str, JsonValue]
) -> None:
...
async def test_non_finite_vector_maps_to_invalid_response(build: Fixture[Build]) -> None:
...
async def test_malformed_json_maps_to_invalid_response(build: Fixture[Build]) -> None:
...
async def test_empty_summary_maps_to_invalid_response(build: Fixture[Build]) -> None:
    ai, _ = build(
...
async def test_transport_errors_retry_once_then_report_unavailable(build: Fixture[Build]) -> None:
    seen: list[httpx.Request] = []
...
async def test_retryable_status_succeeds_on_the_second_attempt(build: Fixture[Build]) -> None:
...
async def test_ok_status_with_error_envelope_is_retried_then_mapped(build: Fixture[Build]) -> None:
...
async def test_persistent_error_envelope_reports_the_inner_status(build: Fixture[Build]) -> None:
